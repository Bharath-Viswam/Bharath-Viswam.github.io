...
let regexp_obj = /^([A-Za-z0-9\.-]+)@([A-Za-z0-9\-]+).([a-z]{2,3})(.[a-z]{2,3})?$/; //ph_no reg exp
...
function validation_snup() {
	...
	if (regexp_obj__test.test(paswd_signup.value.trim())) {
		...
		if (email_signup.value.trim() == '' && paswd_signup.value.trim() == '' && paswd_signup.value.trim() == '') {
			...
			phnmb_signuplbl.innerText = 'phonenumber field should not be empty';
			phnmb_signuplbl.style.color = 'red';
			return false;
		} else if (email_signup.value.trim() == '') {
